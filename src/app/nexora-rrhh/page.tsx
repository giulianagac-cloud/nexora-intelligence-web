  ...
  return (
    <main style={{ background: 'var(--black)', color: 'var(--white)', fontFamily: 'var(--mono)' }}>
      ...
      <section className="pg-sec" style={{ background: 'var(--deep)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ fontFamily: 'var(--mono)', fontSize: '12px', letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '18px' }}>// tres pilares</div>
        ...
        <div className="pg-grid-3c">
          {[
            { num: '01', title: 'Reclutamiento', desc: 'Publicá vacantes, manejá candidatos en un pipeline Kanban y dejá que la IA haga el scoring automático. Menos tiempo revisando CVs, más tiempo hablando con las personas correctas.' },
            { num: '02', title: 'Onboarding', desc: 'Cuando contratás a alguien, el sistema arranca solo. Emails automáticos, accesos configurados, documentación enviada. El nuevo empleado llega el primer día con todo listo.' },
            { num: '03', title: 'Gestión diaria', desc: 'Consultas de vacaciones, licencias, avisos de enfermedad — respondidas por el chatbot sin intervención humana. Tu equipo de RRHH se enfoca en lo que importa.' },
          ].map(s => (
            <div key={s.num} style={{ background: 'var(--surface)', padding: '42px 34px 52px', position: 'relative' }}>
              <div style={{ position: 'absolute', top: '-10px', right: '16px', fontFamily: 'var(--hl)', fontSize: '120px', fontWeight: 700, color: 'rgba(139,92,246,.05)', pointerEvents: 'none', lineHeight: 1 }}>{s.num}</div>
